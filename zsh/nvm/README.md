...
function for `node`, `npm`, `npx`, `pnpm`, `pnpx`, `yarn`, `corepack` and the
command(s) specified by `lazy-cmd` option, so when you call either of them, nvm
will be loaded and run with default version. To enable it, you can add this
snippet to your zshrc, before Oh My Zsh is sourced:
...
There will be a function `_omz_nvm_load` available to load `nvm` without
executing any other trigger command.
